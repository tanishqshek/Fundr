...
func TestPostPitch(t *testing.T) {
	...
	req_postpitch, _ := http.NewRequest("POST", "/api/auth/getuserdata", bytes.NewBuffer(pitchdata_json))
	...
	req_postpitch.AddCookie(&http.Cookie{Name: "mysession", Value: cookie_val})
	router.ServeHTTP(w_userdata, req_postpitch)
	...
	// json.Unmarshal(w_userdata.Body.Bytes(), &resp)
	// if err != nil {
	// 	log.Println(err.Error())
	// }
